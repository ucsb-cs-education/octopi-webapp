...
describe SchoolClass do
  ...
  describe '#teachers' do
    let(:teacher_in_the_school_class) { FactoryGirl.create(:user, :teacher, school_class: @school_class) }
    let(:teacher_a_different_school_class) { FactoryGirl.create(:user, :teacher, school_class: FactoryGirl.create(:school_class)) }
    it 'should contain teachers in the school class' do
      expect(@school_class.teachers.include?(teacher_in_the_school_class)).to be_true
    end
    it 'should not contain teachers in a different school class' do
      expect(@school_class.teachers.include?(teacher_a_different_school_class)).to be_false
    end
  ...
  describe '#students' do
    ...
    it 'should contain students in the school class' do
      expect(@school_class.students.include?(student_in_the_school_class)).to be_true
    end
    it 'should not contain students in a different school class' do
      expect(@school_class.students.include?(student_in_a_different_school_class)).to be_false
    end
  end
