...
class Staff::StaticPagesController < ApplicationController
  ...
  before_action :setup_laplaya, only: [:laplaya, :laplaya_file, :readonlylaplaya_file]
  before_action :set_developer_mode, only: [:laplaya, :laplaya_file, :readonlylaplaya_file]
  before_action :force_trailing_slash, only: [:laplaya]
  before_action :force_no_trailing_slash, only: [:laplaya_file,:readonlylaplaya_file]
  ...
  def readonlylaplaya_file
    authorize! :show, LaplayaFile.select(:id,:type).find(params[:id])
    @laplaya_ide_params[:fileID] = params[:id]
    @laplaya_ide_params[:readOnly] = true;
    staff_laplaya_helper
  end
